...
#include <vector>
#include <iostream>
#include <algorithm> // for sorting chars in a string
#include <sstream>
#include <climits>
...
void QueryOptimizer::Partition(string tables)
{
	string copy = tables;
	...
	map<string,plan>::iterator it = Map.find(copy);
	...
	{	
		for (int j = 0; j <= N - 2; j++)
		{
			...
			if (cost < min_cost){
				size = Map[left].size*Map[right].size;
